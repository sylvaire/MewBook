...
一个基于 **Jetpack Compose + Material 3** 的 Android 记账应用，支持多账本、分类管理、预算管理、统计分析、资产账户、周期模板、删除找回、数据迁移、智能导入、WebDAV 同步与应用内更新。

## 功能概览

- 首页记账：新增、编辑、删除记录（移入本地 30 天回收站），支持快捷记账、流水详情、内联搜索与日/周/月周期切换
- 分类管理：单层收支分类维护、语义匹配、扩展图标选择与排序；记账页新增记录的分类选择与分类管理保持一致
- 资产账户：账户新增、编辑、余额联动与账本关联
- 账本管理：默认账本、排序与多账本数据隔离
- 预算管理：总预算、分类预算、不同预算周期与余额提醒
- 周期模板：工资、房租、订阅等固定收入/支出模板
- 统计报表：周/月/年维度趋势、分类占比与支出分类明细下钻（支持点击查看、编辑和删除记录，删除后可找回）
- 删除找回：设置页回收站可恢复 30 天内删除的记录，或永久删除；回收站数据仅本地保留，不参与普通备份导出
- 数据迁移：本地备份/还原、CSV/JSON 导出、外部导入预览与智能导入，完整恢复和清除数据会清空回收站
- WebDAV 同步：远端备份、手动导出自定义文件名（自动加 `manual_` 前缀）、手动选择远端备份恢复、连接探测、同步预览、导入前自动安全备份与每日首次打开自动备份（失败自动重试）
- 应用更新：版本详情弹窗展示项目 GitHub 仓库链接，保留手动检查更新；支持 GitHub Release 检查、APK 下载、系统安装器引导、跳过此版本和关闭自动检查
- 触感反馈：记账键盘、设置操作、周期切换和快捷入口提供可关闭的震动反馈
- 主题模式：浅色 / 深色 / 跟随系统
...
- `Settings`：主题、首页偏好、触感反馈、账本、分类、预算、周期模板、回收站、迁移备份、DAV、版本详情、自动更新、清除数据入口
- `Export` / `SmartImport`：备份还原、格式导出、外部数据导入与智能导入
- `RecurringTemplates`：固定记账模板管理

## 2026-05 当前更新

- 分类模型清理为单层结构，旧的父子分类字段不再进入当前 Room/备份模型；旧备份和 CSV 中的二级分类会按最终分类名导入。
- 分类管理去除残留的二级默认项、颜色圆点和右侧编辑图标，补充汽车、交通、医疗、购物、家居、金融等常用图标，并优化为与整体 Clay 风格一致的卡片界面。
- 记账页新增记录的分类选择与分类管理共用同一套可见分类；编辑旧记录时仅保留当前已选的退休分类，避免历史记录丢失上下文。
- 设置页点击版本号改为打开应用详情弹窗，弹窗内提供项目 GitHub 仓库链接和“检查更新”；设置列表中不再单独保留手动检查更新项。
- “按键震动”升级为“触感反馈”，覆盖记账键盘、设置行、开关、弹窗、周期选择和快捷入口等合适点击事件。
- 删除记录会移入本地回收站，30 天内可在设置页找回；恢复会重新计入首页、统计和账户余额。
- 回收站记录不参与首页、统计、CSV/JSON/DAV 普通备份导出；完整恢复备份和“清除所有数据”会清空回收站。
- 删除确认与提示条已改为可找回语义，提示“已移入回收站”。
